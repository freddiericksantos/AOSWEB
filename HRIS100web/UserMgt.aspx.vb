...
Public Class UserMgt
    ...
    Private Sub dgvSideListView_SelectedIndexChanged(sender As Object, e As EventArgs) Handles dgvSideListView.SelectedIndexChanged
        'If dgvSideListView.Rows.Count = 0 Then
        '    Exit Sub
        'End If

        'txtUserID.ReadOnly = False
        'txtUserID.Text = dgvSideListView.SelectedRow.Cells(1).Text
        admCurrID = dgvSideListView.SelectedRow.Cells(1).Text
        TextBox3.Text = dgvSideListView.SelectedRow.Cells(1).Text
        'lblMsg.Text = dgvSideListView.SelectedRow.Cells(1).Text
        LoadUserDetails()
    ...
    Private Sub LoadUserDetails()
        'admCurrID = dgvSideListView.SelectedRow.Cells(1).Text
        'lblMsg.Text = admCurrID

        dt = GetDataTable("select userfullname,userpassword,usergroup,useremailaddress,usertelno,usernotes,userloc,smnno,empno,ifnull(wfh,'No')," &
                          "username from sys_userrecords where username = '" & dgvSideListView.SelectedRow.Cells(1).Text & "'")
        If Not CBool(dt.Rows.Count) Then
            Exit Sub
        Else
            For Each dr As DataRow In dt.Rows
                txtUserFullname.Text = dr.Item(0).ToString() & ""
                'TextBox2.Text = base64Encode(dr.Item(1).ToString())

                cboUserGroup.Text = dr.Item(2).ToString() & ""
                txtUserEmailAdd.Text = dr.Item(3).ToString() & ""
                txtUserTelNo.Text = dr.Item(4).ToString() & ""
                txtUserNotes.Text = dr.Item(5).ToString() & ""
                ...
                txtSmnNo.Text = dr.Item(7).ToString() & ""
                ...
                txtEmpNo.Text = dr.Item(8).ToString() & ""

                Select Case dr.Item(9).ToString() & ""
                    Case "Yes"
                ...
                'txtUserID.Text = dr.Item(10).ToString() & ""
                'txtUserPass.Text = base64Encode(dr.Item(1).ToString())
            Next
    ...
    Protected Sub OnConfirm(sender As Object, e As EventArgs)
        ...
        If confirmValue = "Yes" Then
            ...
            sql = "update sys_userrecords set userpassword = '" & base64Encode(admResPass) & "' " &
                  "where username = '" & admCurrID & "'"
            ExecuteNonQuery(sql)

            lblMsg.Text = "User ID: " & admCurrID & " Password RESET"
    ...
    Private Sub SaveUserProc()
        ...
        dt = GetDataTable("select userfullname from sys_userrecords where username = '" & TextBox3.Text & "'")
        If Not CBool(dt.Rows.Count) Then
            SaveUser()
    ...
    Private Sub SaveUser()
        sql = "insert into sys_userrecords(username,userfullname,userpassword,usergroup,useremailaddress,usertelno," &
              "usernotes,userloc,stat,smnno,empno,wfh,user,pdate)values('" & TextBox3.Text & "','" & txtUserFullname.Text & "'," &
    ...
    Private Sub UpdateUser()
        sql = "update sys_userrecords set userfullname = '" & txtUserFullname.Text & "',usergroup = '" & cboUserGroup.Text & "'," &
              ...
              "pdate = '" & Format(CDate(Now()), "yyyy-MM-dd HH:mm:ss") & "' where username = '" & TextBox3.Text & "'"
        ExecuteNonQuery(sql)
